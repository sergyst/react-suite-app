...
export function mockUsers(length) {
  ...
  const createRowData = rowIndex => {
    const firstName = Math.floor(Math.random() * 100)+" Sergey";
    const lastName = Math.floor(Math.random() * 100)+" Steblin";
    const age = Math.floor(Math.random() * 30) + 18;
    const loanStatus = (age % 3) + 1; // === 0) ? "active" : "wait approve";
